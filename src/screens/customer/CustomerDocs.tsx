...
export function CustomerDocs() {
  ...
  const download = (d: CustomerDocument) => { downloadPDF(`${d.id}.pdf`, customerReceiptDoc(d)); toast(`${t("Downloaded")} ${d.id}.pdf`, { tone: "success" }); };
  const downloadAll = () => {
    const files = filtered.map((d) => ({ name: `${d.id}.pdf`, content: buildReceiptBytes(d) }));
    if (!files.length) { toast(t("Nothing to bundle in this filter.")); return; }
    downloadZIP(`slaice-receipts-${new Date().toISOString().slice(0,10)}.zip`, files);
    toast(`${t("Bundled")} ${files.length} PDF${files.length === 1 ? "" : "s"} ${t("into ZIP.")}`, { tone: "success" });
  };
    ...
    <div className="space-y-4">
      {docsQ.status === "loading" ? (
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          <StatCard label={t("Receipts this season")} value={docs.length} sub={`${docs.filter((d) => tone(d.id) === "apy").length} ΑΠΥ`} tone="teal" />
          <StatCard label={t("Total spend")} value={`€${totalAmount}`} sub={t("all paid · MyDATA ✓")} />
          <StatCard label={t("MyDATA status")} value="100%" sub={t("transmitted")} tone="indigo" />
        </div>
      )}
      <Card className="p-4">
        <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
          <Tabs tabs={[["all", t("All")], ["apy", "ΑΠΥ"], ["tpy", "ΤΠΥ"], ["credit", t("Credit notes")]]} value={filter} onChange={setFilter} />
          <Btn size="sm" variant="outline" icon={Icon.download} onClick={downloadAll}>{t("Download all (ZIP)")}</Btn>
        </div>
        {docsQ.status === "loading" ? (
          <TableSkeleton rows={2} cols={6} />
        ) : docsQ.status === "error" ? (
          <ErrorState compact body={t("We couldn't load your documents.")} onRetry={docsQ.refetch} />
        ) : filtered.length === 0 ? (
          <EmptyState icon={Icon.receipt} title={t("No documents in this filter")} body={t("Try selecting another category.")} />
        ) : (
          <Table cols={[t("Document"), t("For"), t("Date"), t("Amount"), t("Status"), ""]} right={[3]}
            rows={filtered.map((d) => [d.id, d.for, d.date, d.amt, <StatusBadge status={t("MyDATA ✓")} />,
              <span className="flex gap-1 justify-end">
                <Btn size="sm" variant="ghost" icon={Icon.doc} onClick={() => setView(d)}>{t("View")}</Btn>
                <Btn size="sm" variant="ghost" icon={Icon.download} onClick={() => download(d)}>PDF</Btn>
              </span>])} />
        )}
      </Card>
      <Modal open={!!view} onClose={() => setView(null)} title={view?.id ?? t("Document")}
        footer={<>
          <Btn variant="ghost" onClick={() => setView(null)}>{t("Close")}</Btn>
          <Btn variant="primary" icon={Icon.download} onClick={() => { if (!view) return; download(view); setView(null); }}>{t("Download")}</Btn>
        </>}>
        {view && (
          <div className="text-sm">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-3">
              ...
              <Badge tone="green">{t("MyDATA ✓")}</Badge>
            </div>
            ...
            <div className="mt-3 pt-3 border-t border-slate-100 flex justify-between font-semibold text-navy-900"><span>{t("Total gross")}</span><span className="tnum">{view.amt}</span></div>
